(() => {
  ...
  let abyssShocks=[];
  let kawazuShots=[];
  // Water Fighter 2 prototype: simple readable projectiles with wind-up.
  let water2Shots=[];
  let kawazuGhosts=[];
  let siltClouds = [];
  let catfishCharges = [];
  ...
  class Fighter {
    ...
    draw() {
      ...
      const eyeTarget=this.isPlayer?enemy:player;
      const eyeDx=eyeTarget?Math.max(-4,Math.min(4,(eyeTarget.x-this.x)/70)):0;
      const eyeDy=eyeTarget?Math.max(-2.5,Math.min(2.5,(eyeTarget.y-this.y)/100)):0;

      // Water Fighter 2: a large transparent bubble makes guarding/reflecting obvious.
      if(this.guard && this.type!=='piranha' && this.type!=='crayfish'){
        const pulse=1+Math.sin(performance.now()/95)*.018;
        ctx.save(); ctx.scale(pulse,pulse);
        ctx.fillStyle='rgba(210,248,255,.10)';
        ctx.strokeStyle='rgba(225,252,255,.82)'; ctx.lineWidth=3;
        ctx.shadowColor='rgba(120,225,255,.75)'; ctx.shadowBlur=16;
        ctx.beginPath(); ctx.arc(0,8,this.radius+24,0,Math.PI*2); ctx.fill(); ctx.stroke();
        ctx.restore();
      }
      ...
      if(this.hurtFaceT>0 || this.throwState){
        ...
        ctx.fillStyle='#182a2a';
        ctx.beginPath();
        ctx.arc(-19+eyeDx,-30+eyeDy,4,0,Math.PI*2);
        ctx.arc(19+eyeDx,-30+eyeDy,4,0,Math.PI*2);
        ctx.fill();
  ...
  function hasBackBackCommand(f, windowMs=820){
    ...
    // Water Fighter 2: basic shots are intentionally easy; stronger moves keep harder inputs.
    if(kind==='punch' && f.type==='green' && hasCommand([back],520)) return spawnWater2Shot(f,kind);
    if(kind==='punch' && f.type==='blue' && hasCommand([forward],520)) return spawnWater2Shot(f,kind);
    if(kind==='kick' && f.type==='black' && hasCommand([forward],520)) return spawnWater2Shot(f,kind);
    if(kind==='tongue' && f.type==='purple' && hasCommand([back],520)) return spawnWater2Shot(f,kind);
    if(kind==='punch' && f.type==='orange' && f.guard) return spawnWater2Shot(f,kind);
  ...
  function water2ReflectProjectile(p,target){
    if(!p||!target||!target.guard||typeof p.vx!=='number')return false;
    p.owner=target; p.hit=false;
    p.vx=-p.vx*1.10; p.vy=(p.vy||0)*1.04;
    p.r=Math.min(30,(p.r||10)*1.05);
    p.x=target.x+Math.sign(p.vx||target.face)*(target.radius+(p.r||10)+12);
    p.y=target.y;
    spawnImpact(target.x,target.y,'guard'); playSfx('guard');
    comboEl.textContent='REFLECT!';
    return true;
  }

  function spawnWater2Shot(f,kind){
    if(!f||f.stun>0||f.specialT>0)return false;
    const cfg={
      green:{name:'バーニングショット!',fill:'#ff6b35',speed:350,dmg:4.2,r:12},
      blue:{name:'アクアショット!',fill:'#72e8ff',speed:330,dmg:3.8,r:12},
      black:{name:'パワーショット!',fill:'#a97cff',speed:300,dmg:5.0,r:14},
      purple:{name:'バブルショット!',fill:'#f2a5ff',speed:235,dmg:3.2,r:15},
      orange:{name:'ホワイトショット!',fill:'#ffffff',speed:320,dmg:3.8,r:12}
    }[f.type];
    if(!cfg)return false;
    f.specialType='water2ShotWindup'; f.specialT=.38; f.attack=kind; f.attackT=.38;
    comboEl.textContent=cfg.name.replace('!','')+'…';
    setTimeout(()=>{
      if(gameOver||!f)return;
      water2Shots.push({owner:f,x:f.x+f.face*46,y:f.y-2,vx:f.face*cfg.speed,vy:0,r:cfg.r,t:2.8,hit:false,dmg:cfg.dmg,fill:cfg.fill});
      comboEl.textContent=cfg.name;
    },260);
    clearCommand(); return true;
  }
    ...
    if(!gameOver){
      ...
      engineerShots.forEach(q=>{
        ...
        if(target&&!q.hit&&Math.hypot(target.x-q.x,target.y-q.y)<target.radius+q.r+4){
          if(water2ReflectProjectile(q,target)) return;
          q.hit=true;q.t=0;
      ...
      michaelAuraShots.forEach(s=>{
        ...
        if(target&&!s.hit&&Math.hypot(target.x-s.x,target.y-s.y)<target.radius+s.r+8){
          if(water2ReflectProjectile(s,target)) return;
      ...
      water2Shots.forEach(p=>{
        p.t-=dt; p.x+=p.vx*dt; p.y+=(p.vy||0)*dt;
        const target=p.owner&&p.owner.isPlayer?enemy:player;
        if(!p.hit&&target&&Math.hypot(target.x-p.x,target.y-p.y)<target.radius+p.r+5){
          if(!water2ReflectProjectile(p,target)){
            p.hit=true; p.owner._projectileHit=true;
            damageHit(p.owner,target,p.dmg*p.owner.damageMul,70*Math.sign(p.vx),-8);
            p.owner._projectileHit=false; spawnImpact(p.x,p.y,'hit');
          }
        }
      });
      water2Shots=water2Shots.filter(p=>p.t>0&&!p.hit&&p.x>-80&&p.x<innerWidth+80);
      ...
      abyssShocks.forEach(w=>{
        ...
        if(!w.hit && target && Math.hypot(target.x-w.x,target.y-w.y)<target.radius+w.r){
          if(water2ReflectProjectile(w,target)) return;
          w.hit=true;
      ...
      water2Shots.forEach(p=>{
        ctx.save(); ctx.globalCompositeOperation='lighter';
        ctx.fillStyle=p.fill||'#dffaff'; ctx.shadowColor=p.fill||'#dffaff'; ctx.shadowBlur=18;
      ...
      kawazuShots.forEach(p=>{
        ...
        if(!p.hit&&target&&Math.hypot(target.x-p.x,target.y-p.y)<target.radius+p.r){
          if(water2ReflectProjectile(p,target)) return;
          p.hit=true;
